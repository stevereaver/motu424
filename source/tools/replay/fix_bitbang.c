...
int main(int argc, char *argv[]) {
    ...
    printf("Final Corrected FPGA Upload (Clock returning to LOW)...\n");
    ...
    for (size_t i = 0; i < fw_size; i++) {
        ...
        for (int bit = 0; bit < 8; bit++) {
            ...
            uint32_t val_low = 0x40 | data_bit;
            uint32_t val_high = 0xC0 | data_bit;
            
            write_reg(fd, 1, 0x300008, val_low);
            write_reg(fd, 1, 0x300008, val_high);
            write_reg(fd, 1, 0x300008, val_low); // CRITICAL: RETURN TO LOW
        }
        if (i % 1000 == 0) printf("\rProgress: %zu/%zu", i, fw_size);
    }
    write_reg(fd, 1, 0x300004, 0xC0); 
    write_reg(fd, 2, 0x8, 0x0);
    printf("\nFirmware upload complete. Watch the rack unit!\n");
